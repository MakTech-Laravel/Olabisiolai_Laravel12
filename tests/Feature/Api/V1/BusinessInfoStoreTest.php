...
class BusinessInfoStoreTest extends TestCase
{
    ...
    public function test_verified_vendor_can_create_business_profile_with_uploads(): void
    {
        ...
        $response = $this->withToken($token)->post('/api/v1/vendor/business/create', [
        ...
        $logoUpload = $this->withToken($token)
            ->post('/api/v1/media', [
                'file' => UploadedFile::fake()->image('logo.png', 100, 100),
                'uploadable_type' => 'business',
                'uploadable_id' => $business->id,
            ]);
        $logoUpload->assertCreated();

        $coverUpload = $this->withToken($token)
            ->post('/api/v1/media', [
                'file' => UploadedFile::fake()->image('cover1.png', 200, 200),
                'uploadable_type' => 'business',
                'uploadable_id' => $business->id,
            ]);
        $coverUpload->assertCreated();

        $attach = $this->withToken($token)->putJson('/api/v1/vendor/business/update', [
            'logo_path' => $logoUpload->json('data.path'),
            'keep_cover_paths' => [],
            'cover_photo_paths' => [$coverUpload->json('data.path')],
        ]);
        $attach->assertOk();
    ...
    public function test_second_create_returns_error(): void
    {
        ...
        $this->withToken($token)->post('/api/v1/vendor/business/create', $payload)->assertCreated();

        $payload['business_name'] = 'Second Co';
        $this->withToken($token)->post('/api/v1/vendor/business/create', $payload)
            ->assertUnprocessable()
    ...
    public function test_verified_vendor_can_edit_business_profile_without_changing_url(): void
    {
        ...
        $response = $this->withToken($token)->putJson('/api/v1/vendor/business/update', [
