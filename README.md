...
The marketplace sitemap is generated by Laravel (DB + visibility rules) but `<loc>` values use **`FRONTEND_URL`** (SPA host), not `APP_URL`.

- **Generate:** `php artisan sitemap:generate` (scheduled daily), or **Admin → SEO → Generate sitemap**
- **Serve:** `GET /sitemap.xml` on the API host (e.g. `https://api.gidira.tech/sitemap.xml`)
- **Output file:** `storage/app/sitemap/sitemap.xml`
- **robots.txt:** API `public/robots.txt` includes `Sitemap: https://api.gidira.tech/sitemap.xml`
- **Admin SEO:** seeded `seo_pages` (from `config/sitemap-urls.php`) — edit meta title/description/keywords; public `GET /api/v1/seo-pages/by-path?path=/about` for SPA `DocumentHead`
- **SPA follow-up:** the frontend deploy should also advertise the same `Sitemap:` line in its `robots.txt` / nginx config (that config may live outside this repo)
- **Cache follow-up:** if public meta is ever wrapped in `Cache::remember`, invalidate on admin SEO update
