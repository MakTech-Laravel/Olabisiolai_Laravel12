...
The marketplace sitemap is generated by Laravel (DB + visibility rules) but `<loc>` values use **`FRONTEND_URL`** (SPA / www host), not `APP_URL`.

- **Generate / refresh:** `php artisan sitemap:generate` (daily), `php artisan sitemap:refresh` (hourly cache warm), or **Admin → SEO → Generate sitemap**
- **Serve:** `GET /sitemap.xml` and `GET /robots.txt` on the API; **www nginx proxies** both so Search Console can use the content origin
- **Output file:** `storage/app/sitemap/sitemap.xml` (+ HTTP `Cache::remember`)
- **robots.txt:** dynamic route (static `public/robots.txt` removed). `Sitemap:` uses `FRONTEND_URL/sitemap.xml`. Disallows admin/vendor/auth utility paths
- **Resolve:** `GET /api/v1/seo-pages/resolve?path=` — single source for spa-shell + SPA `DocumentHead` (title, robots, OG/Twitter, JSON-LD). Legacy `by-path` still exists
- **Admin SEO:** `seo_pages` meta + `noindex` / `canonical_url` / `og_image`; noindex pages are omitted from the sitemap
- **No** Google News `/news-sitemap.xml` (not a news product)
- **spa-shell:** `GET /spa-shell?path=` injects the same resolved head into the SPA HTML shell
